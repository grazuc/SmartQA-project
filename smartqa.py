"""
Smart Doc QA - CLI tool for document question answering
"""
...
class SmartQA:
    ...
    def __init__(self, chunk_size: int = None, chunk_overlap: int = None):
        ...
        # Initialize document processor, added optional chunk_size and chunk_overlap (and default sizes also)
...
Answer (max 200 words):""",
        ...
        # Simple confidence scoring prompt for bonus feature
        self.confidence_prompt = PromptTemplate(
            template="""How certain are you about the previous answer on a scale of 0-1? 
Answer: {answer}
Context: {context}
...
Confidence (0-1): """,
    ...
    def ask_question(self, question: str, include_confidence: bool = False) -> QAResult:
        ...
        if include_confidence:
            try:
                conf_chain = LLMChain(llm=self.llm, prompt=self.confidence_prompt)
                conf_result = conf_chain.invoke({"answer": answer, "context": context})
                confidence = float(conf_result["text"].strip())
    ...
    def display_result(self, question: str, result: QAResult):
        ...
        if result.confidence is not None:
            ...
            answer_content += f"\n\n[{confidence_color}]Confidence: {result.confidence:.2f}[/{confidence_color}]"
...
def main(
    ...
    score: bool = typer.Option(False, "--score", help="Include confidence scoring (bonus feature)"),
    ...
    try:
        ...
        if ask:
            ...
            result = qa_system.ask_question(ask, include_confidence=score)
            ...
            while True:
                try:
                    ...
                    result = qa_system.ask_question(question, include_confidence=score)
